...
# 변경 사항: 두 개의 "코드 작성" 중 첫 번째(인덱스 1)를 "코드 버그 수정"으로 변경하고 프롬프트 내용을 수정함.
prompts = [
    {
        "title": "코드 설명",
...
for i, item in enumerate(prompts):
    # HTML 앵커 링크를 사용해 같은 페이지 내 해당 부분으로 스크롤
    st.sidebar.markdown(f"[{item['title']}](#prompt-{i})", unsafe_allow_html=True)
